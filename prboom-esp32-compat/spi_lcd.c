...
// Default LCD GPIO pins for ESP-WROVER-KIT V2/V3
// These can be overridden via Kconfig if available
#ifndef CONFIG_HW_LCD_MOSI_GPIO
#define CONFIG_HW_LCD_MOSI_GPIO 23
#endif
#ifndef CONFIG_HW_LCD_MISO_GPIO
...
// ST7789 240x280 in LANDSCAPE mode (rotated 90°)
...
#if (CONFIG_HW_LCD_TYPE == 1)
// ST7789V 240x280 initialization
static const ili_init_cmd_t ili_init_cmds[]={
    // Software reset
    {0x01, {0}, 0x80},
    // Sleep out
    {0x11, {0}, 0x80},
    // Memory Data Access Control - set rotation/orientation
    // Bits: MY MX MV ML BGR MH 0 0
    // 0x60 = MX=1, MV=1 -> Landscape mode (rotate 90° clockwise)
    // 0xA0 = MY=1, MV=1 -> Landscape mode (rotate 90° counter-clockwise)
    // Using 0x60 for landscape with correct orientation
    {0x36, {0x60}, 1},
    ...
    // Display Inversion On (ST7789 typically needs this for correct colors)
    {0x21, {0}, 0},
    // Display on
    {0x29, {0}, 0x80},
    {0, {0}, 0xff}
...
#if (CONFIG_HW_LCD_TYPE == 1)
// Use full 280x240 display - stretch to fill
#define SCALED_WIDTH  280
...
#else
		// ILI9341 320x240: Original code path
...
void spi_lcd_init() {
	printf("spi_lcd_init()\n");
	printf("LCD Type: %d, Size: %dx%d\n", CONFIG_HW_LCD_TYPE, LCD_WIDTH, LCD_HEIGHT);
